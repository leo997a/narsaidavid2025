...
import { CalendarIcon, Upload, ImageIcon, LogOut, Save, RefreshCw, ArrowLeft, Database, CloudUpload } from "lucide-react";
...
const Admin = () => {
  ...
  // تعيين المستخدم كمسؤول عند فتح صفحة الإدارة
  useEffect(() => {
    setIsAdmin(true);
    ...
    // تحديث الترتيب
    calculateStandings();
  }, [setIsAdmin, syncData, isSynced, calculateStandings]);
  ...
  const handleSaveAllChanges = () => {
    saveAllData();
    showSaveSuccess('تم حفظ ومزامنة جميع التغييرات بنجاح');
  };
  
  // مزامنة البيانات من الخادم
  const handleSyncData = () => {
    syncData();
    toast.success("جاري مزامنة البيانات من الخادم...");
  };
    ...
    <Layout>
      <div className="mb-6 flex flex-wrap justify-between items-center gap-2">
        ...
        <div className="flex gap-2 items-center flex-wrap">
          ...
          >
            <CloudUpload size={16} />
            <span>{isLoading ? 'جاري الحفظ...' : (saveSuccess ? 'تم الحفظ!' : 'حفظ ومزامنة')}</span>
          </Button>
            ...
            <div className="glassmorphism p-6 rounded-lg">
              ...
              <div className="space-y-4">
                <p className="text-gray-300">
                  يمكنك حفظ التغييرات ومزامنتها مع الخادم لتكون متاحة لجميع المستخدمين. البيانات محفوظة في قاعدة بيانات Firebase.
                </p>
                
                <div className="flex flex-wrap items-center gap-4 mt-4">
                  <div className="bg-tournament-navy p-3 rounded-lg flex items-center gap-2">
                    <div className={`w-3 h-3 rounded-full ${isSynced ? 'bg-green-500' : 'bg-yellow-500'}`}></div>
                    <span>
                      {isSynced ? 'البيانات متزامنة مع الخادم' : 'البيانات غير متزامنة'}
                    </span>
                  </div>
                  ...
                  >
                    <CloudUpload size={16} />
                    <span>حفظ ومزامنة البيانات</span>
                  </Button>
                </div>
              </div>
            </div>
